...
define concat(
  ...
  $order='alpha'
    ...
    exec{"concat_${name}":
        ...
        unless    => "${concat::setup::concatdir}/bin/concatfragments.sh \
                      -o ${fragdir}/${concat_name} \
                      -d ${fragdir} -t ${warnflag} ${forceflag} ${orderflag} \
                      ${gnuflag}",
        command   => "${concat::setup::concatdir}/bin/concatfragments.sh \
                      -o ${fragdir}/${concat_name} \
                      -d ${fragdir} ${warnflag} ${forceflag} ${orderflag} \
                      ${gnuflag}",
    }
    if $::id == 'root' {
      Exec["concat_${name}"]{
